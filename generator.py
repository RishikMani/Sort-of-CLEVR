...
def build_dataset(dataset_type, count):
    ...
    for color_id, color in enumerate(colors):
        ...
        if random.random() < 0.5:
            ...
            end = (center[0] + size, center[1] + size)    # opposite vertex
        ...
        if dataset_type == "test":
            cv2.imwrite(test_images + "/img_" + str(count).zfill(4) + ".jpeg",
                        img)
        else:
            cv2.imwrite(train_images + "/img_" + str(count).zfill(4) + ".jpeg",
                        img)
    ...
    for question in chosen_questions:
        ...
        if question["id"] == 1:
            question["question"] = \
                question["question"].replace("<C>", colors_code[color])
        ...
        # Is the <C> colored object square shaped?
        elif question["id"] == 6:
            question["question"] = \
                question["question"].replace("<C>", colors_code[color])
        ...
        # Is the <C> colored object circle shaped?
        elif question["id"] == 7:
            question["question"] = \
                question["question"].replace("<C>", colors_code[color])
        ...
        # Are there more than one object of similar shape as of <C> colored
        # object?
        elif question["id"] == 9:
            question["question"] = \
                question["question"].replace("<C>", colors_code[color])
        ...
        elif question["id"] in (20, 21, 22, 23):
            question["question"] = \
                question["question"].replace("<C>", colors_code[color])
    ...
    if dataset_type == "test":
        ...
        question_ids_file = "./data/test/test_question_ids.txt"
        answers_file = "./data/test/test_answer.txt"
    else:
        questions_file = "./data/train/train_questions.txt"
        question_ids_file = "./data/train/train_question_ids.txt"
    ...
    with open(question_ids_file, 'a') as f:
