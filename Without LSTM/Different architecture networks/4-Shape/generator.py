...
def generator(config):
    ...
    def generate_answer(rep):
        ...
        for i in range(NUM_SHAPE):
            ...
            # Q6: the shape of the nearest object
            if rep.shape[min_idx]:
                A[i*NUM_Q+5, NUM_COLOR] = True
            else:
                A[i*NUM_Q+5, NUM_COLOR+1] = True
            # Q7: the shape of the farthest object
            if rep.shape[max_idx]:
                A[i*NUM_Q+6, NUM_COLOR] = True
            else:
                A[i*NUM_Q+6, NUM_COLOR+1] = True
            # Q8: is the shape of the nearest object same?
            if rep.shape[i] == rep.shape[min_idx]:
                A[i*NUM_Q+7, NUM_COLOR+2] = True
            else:
                A[i*NUM_Q+7, NUM_COLOR+3] = True
            # Q9: is the shape of the farthest object same?
            if rep.shape[i] == rep.shape[max_idx]:
                A[i*NUM_Q+8, NUM_COLOR+2] = True
            else:
                A[i*NUM_Q+8, NUM_COLOR+3] = True
        return A
